...
std::string _distributed_plugin{"bro"};
...
osquery::Status ZeekLoggerPlugin::setUp() {
  ...
  if (_distributed_plugin != "bro") {
    return osquery::Status::failure("The distributed bro service is disabled");
  }

  if (_logger_event_type) {
    return osquery::Status::failure("Bro logger cannot use event type logging");
  }
...
osquery::Status ZeekLoggerPlugin::logString(const std::string& s) {
  ...
  return broker_manager->logQueryLogItemToBro(item);
}
